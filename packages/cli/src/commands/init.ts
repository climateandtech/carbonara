...
export async function initCommand(options: InitOptions) {
  try {
    ...
    if (isGitRepository(projectPath)) {
      ...
      if (gitRoot) {
        console.log(chalk.gray('📂 Git repository detected, using root:'), gitRoot);
        finalProjectPath = gitRoot;
        dbPath = path.join(gitRoot, 'carbonara.db');
      } else {
        console.log(chalk.yellow('⚠️  Warning: Could not determine git repository root.'));
        console.log(chalk.yellow('   Using current directory instead:'), projectPath);
        dbPath = path.join(projectPath, 'carbonara.db');
      }
    } else {
      console.log(chalk.yellow('⚠️  Warning: No git repository found.'));
      console.log(chalk.yellow('   It is recommended to run'), chalk.white('git init'), chalk.yellow('first.'));
