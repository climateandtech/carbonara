...
async function initCommand(options) {
  try {
    ...
    db.run(
      ...
      [answers.name, projectPath, answers.projectType.value, JSON.stringify({
    ...
    db.run(
      ...
      (err) => {
        ...
        db.run(
          ...
          [config.projectId, 'co2-assessment', 'questionnaire', JSON.stringify({
            ...assessmentData,
            impactScore,
...
function calculateCO2Impact(data) {
  ...
  const hostingMultipliers = { shared: 1, vps: 2, dedicated: 4, cloud: 3, hybrid: 5 };
...
function generateAssessmentReport(data, impactScore) {
  ...
  console.log(`Expected Users: ${data.projectInfo.expectedUsers.toLocaleString()}`);
  ...
  if (data.features.realTimeFeatures) highImpactFeatures.push('Real-time features');
  if (data.features.mediaProcessing) highImpactFeatures.push('Media processing');
  if (data.features.aiMlFeatures) highImpactFeatures.push('AI/ML features');
  if (data.features.blockchainIntegration) highImpactFeatures.push('Blockchain');
  ...
  console.log(chalk.blue('\n🌍 Sustainability:'));
  console.log(`Carbon Neutrality Target: ${data.sustainabilityGoals.carbonNeutralityTarget ? 'Yes' : 'No'}`);
  console.log(`Green Hosting: ${data.sustainabilityGoals.greenHostingRequired ? 'Yes' : 'No'}`);
  console.log(`Optimization Priority: ${data.sustainabilityGoals.optimizationPriority}`);
...
async function greenframeCommand(url, options) {
  try {
    ...
    console.log(chalk.gray('This would normally run Greenframe CLI for carbon footprint analysis.'));
    console.log(chalk.gray('Use the new "carbonara test-website" command for basic website analysis.'));
...
function generateMockWebsiteResults(url) {
  ...
  const requestCount = Math.floor(totalKB / 10) + Math.floor(Math.random() * 20);
...
function displayWebsiteTestResults(results, format) {
  ...
  console.log(chalk.green('📦 Total Data:'), `${chalk.bold(results.totalKB)} KB (${results.totalBytes.toLocaleString()} bytes)`);
  ...
  if (results.breakdown) {
    ...
    Object.entries(results.breakdown).forEach(([type, size]) => {
      console.log(`  ${type.charAt(0).toUpperCase() + type.slice(1)}: ${size} KB`);
    });
  }
  ...
  if (results.performance) {
    ...
    console.log(`  Largest Contentful Paint: ${results.performance.largestContentfulPaint}ms`);
  }

  if (results.carbonEstimate) {
    console.log(chalk.blue('\n🌱 Carbon Estimate:'), `${results.carbonEstimate}g CO2`);
  }
...
async function saveWebsiteTestToDatabase(url, results) {
  try {
    ...
    db.run(
      'INSERT INTO assessment_data (project_id, tool_name, data_type, data, source) VALUES (?, ?, ?, ?, ?)',
      [config.projectId, 'website-test', 'data-transfer-analysis', JSON.stringify({
        url,
        results,
...
async function saveGreenframeToDatabase(url, results) {
  try {
    ...
    db.run(
      'INSERT INTO assessment_data (project_id, tool_name, data_type, data, source) VALUES (?, ?, ?, ?, ?)',
      [config.projectId, 'greenframe', 'web-analysis', JSON.stringify({
        url,
        results,
        analyzedAt: new Date().toISOString()
      }), 'cli'],
          ...
          if (rows.length === 0) {
            ...
            Object.entries(groupedData).forEach(([toolName, data]) => {
              ...
              data.slice(0, 3).forEach((entry, index) => {
                console.log(`  ${index + 1}. ${entry.data_type} - ${new Date(entry.timestamp).toLocaleDateString()}`);
              });
    ...
    db.all(
      ...
      (err, rows) => {
        if (err) reject(err);
          ...
          console.log(`   Name: ${chalk.white(config.name || 'Unnamed Project')}`);
          console.log(`   ID: ${chalk.gray(config.projectId)}`);
          console.log(`   Type: ${chalk.white(config.projectType || 'Unknown')}`);
          ...
          if (groupedData.greenframe) {
            ...
            let data;
            try {
              data = typeof latestGreenframe.data === 'string' ? JSON.parse(latestGreenframe.data) : latestGreenframe.data;
            } catch (e) {
              data = latestGreenframe.data;
            }
            
            console.log(chalk.green.bold('\n🌐 Greenframe Analysis'));
            console.log(`   Date: ${chalk.gray(new Date(latestGreenframe.timestamp).toLocaleDateString())}`);
            console.log(`   URL: ${chalk.blue(data.url || 'N/A')}`);
            
            if (data.carbonFootprint) {
              console.log(`   Carbon Footprint: ${chalk.yellow.bold(data.carbonFootprint)}`);
            }
            if (data.ecoIndex) {
              const ecoColor = data.ecoIndex >= 70 ? 'green' : data.ecoIndex >= 40 ? 'yellow' : 'red';
              console.log(`   Eco Index: ${chalk[ecoColor].bold(data.ecoIndex + '/100')}`);
            }
            if (data.grade) {
          ...
          if (groupedData['website-test']) {
            ...
            let data;
            try {
              data = typeof latestTest.data === 'string' ? JSON.parse(latestTest.data) : latestTest.data;
            ...
            console.log(`   Date: ${chalk.gray(new Date(latestTest.timestamp).toLocaleDateString())}`);
            console.log(`   URL: ${chalk.blue(data.url || 'N/A')}`);
            
            if (data.totalKB) {
              console.log(`   Total Data: ${chalk.yellow.bold(data.totalKB + ' KB')}`);
            }
            if (data.requestCount) {
              console.log(`   Requests: ${chalk.white(data.requestCount)}`);
            }
            if (data.carbonEstimate) {
              console.log(`   Carbon Estimate: ${chalk.green.bold(data.carbonEstimate + 'g CO2')}`);
            }
            if (data.performance && data.performance.loadTime) {
              console.log(`   Load Time: ${chalk.cyan(data.performance.loadTime + 'ms')}`);
            }
          }
          ...
          console.log(chalk.gray(`💡 Use 'carbonara data --export json' to export all data`));
...
function showDetailedHelp() {
  ...
  // Init command
  console.log(chalk.yellow('  init') + '              Initialize new Carbonara project');
  console.log(chalk.gray('                     Creates config, database, and schema files'));
  console.log(chalk.gray('                     Example: carbonara init --path ./my-project\n'));
  
  // Assess command  
  console.log(chalk.yellow('  assess') + '            Run comprehensive CO2 assessment');
  console.log(chalk.gray('                     Interactive questionnaire covering:'));
  console.log(chalk.gray('                     • Project scope (users, traffic, lifespan)'));
  console.log(chalk.gray('                     • Infrastructure (hosting, storage)'));  
  console.log(chalk.gray('                     • Development practices (CI/CD, testing)'));
  console.log(chalk.gray('                     • Features (real-time, AI/ML, blockchain)'));
  console.log(chalk.gray('                     • Sustainability goals'));
  console.log(chalk.gray('                     Example: carbonara assess\n'));
  
  // Greenframe command
  console.log(chalk.yellow('  greenframe <url>') + '   Analyze website carbon footprint');
  console.log(chalk.gray('                     Uses Greenframe CLI for web analysis'));
  console.log(chalk.gray('                     Options: --save (store in database)'));
  console.log(chalk.gray('                     Example: carbonara greenframe https://example.com --save\n'));
  
  // Test Website command
  console.log(chalk.yellow('  test-website <url>') + ' Test website and measure data transfer');
  console.log(chalk.gray('                     Uses Cypress to analyze data usage'));
  console.log(chalk.gray('                     Measures total kilobytes transferred'));
  console.log(chalk.gray('                     Example: carbonara test-website https://example.com --save\n'));
  
  // Data command
  console.log(chalk.yellow('  data') + '              Manage assessment data lake');
  console.log(chalk.gray('                     Options:'));
  console.log(chalk.gray('                     • --list (show all stored data)'));
  console.log(chalk.gray('                     • --show (detailed project analysis)'));
  console.log(chalk.gray('                     • --export json|csv (export data)'));
  console.log(chalk.gray('                     • --clear (delete all data)'));
  console.log(chalk.gray('                     Example: carbonara data --show\n'));
  
  // Help command
  console.log(chalk.yellow('  help [command]') + '     Show help information');
  console.log(chalk.gray('                     Example: carbonara help assess\n'));
  ...
  console.log(chalk.blue('For more information, visit: https://github.com/carbonara/carbonara'));
}
