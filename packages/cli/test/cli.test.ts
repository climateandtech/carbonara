...
describe("Carbonara CLI - Tests", () => {
  ...
  test("greenframe command should work with valid URL", () => {
    try {
      const result = execSync(
        `cd "${testDir}" && node "${cliPath}" greenframe https://example.com`,
        {
          encoding: "utf8",
          timeout: 5000,
        },
  ...
  // MegaLinter tests have been moved to megalinter.test.ts
  // This separation allows for easier exclusion in CI and better test organization
});
