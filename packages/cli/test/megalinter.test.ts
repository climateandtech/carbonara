...
describe("Carbonara CLI - MegaLinter Tests", () => {
  ...
  beforeEach(() => {
    testDir = fs.mkdtempSync(path.join(os.tmpdir(), "carbonara-megalinter-test-"));
  ...
  test("megalinter command should save results to database", () => {
    ...
    if (isCI) {
      ...
      try {
        ...
        expect(fs.existsSync(path.join(reportsDir, "mega-linter-report.json"))).toBe(true);
        console.log("Note: Consider adding --skip-run flag to CLI for CI testing");
      }
  ...
  test("megalinter command should clean up reports folder", () => {
    ...
    if (isCI) {
      ...
      try {
        execSync(`cd "${testDir}" && node "${cliPath}" megalinter --cleanup-only`, {
          encoding: "utf8",
          timeout: 5000,
        });
      } catch {
    ...
    const cleanupOccurred = !fs.existsSync(reportsDir) || 
                           fs.readdirSync(reportsDir).length === 0;
    expect(cleanupOccurred).toBe(true);
  });

  test.skipIf(isCI)("megalinter command should run with custom options (local only)", () => {
    // This test only runs locally as it requires actual megalinter
    fs.writeFileSync(
      path.join(testDir, "carbonara.config.json"),
      JSON.stringify({
        name: "Test Project",
        projectType: "web",
        projectId: "test-megalinter-options",
      }),
    );

    try {
      const result = execSync(
        `cd "${testDir}" && node "${cliPath}" megalinter --flavor documentation`,
        {
          encoding: "utf8",
      ...
      expect(result).toContain("MegaLinter");
      // The actual assertions depend on your CLI implementation
    } catch (error: any) {
      if (error.stderr?.toString().includes("mega-linter-runner: command not found")) {
        console.log("MegaLinter not installed - skipping");
        expect(true).toBe(true);
      } else {
        throw error;
      }
