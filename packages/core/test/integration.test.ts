...
describe("Carbonara Core Integration", () => {
  ...
  describe("End-to-End Data Flow", () => {
    it("should handle complete workflow: project → data → display", async () => {
      ...
      const impactScoreDetail = details.find(
        (d: any) => d.key === "impactScore"
      );
      expect(impactScoreDetail?.label).toContain("75"); // Contains the score value

      const estimatedUsersDetail = details.find(
        (d: any) => d.key === "estimatedUsers"
      );
    ...
    it("should handle data export", async () => {
      ...
      // Test JSON export
      const jsonExport = await vscodeProvider.exportData(
        "/test/export",
        "json"
      );
  ...
  describe("Performance and Scalability", () => {
    it("should handle large datasets efficiently", async () => {
      ...
      for (let i = 0; i < 100; i++) {
        promises.push(
          dataService.storeAssessmentData(
            projectId,
            "greenframe",
            "web-analysis",
            {
      ...
      const groups =
        await vscodeProvider.createGroupedItems("/test/performance");
