...
describe("SchemaService", () => {
  ...
  describe("Data Path Extraction", () => {
    it("should extract value from nested object using path", () => {
      ...
      expect(schemaService.extractValue(data, "data.url")).toBe(
        "https://example.com"
      );
      expect(schemaService.extractValue(data, "data.results.totalBytes")).toBe(
        524288
      );
      expect(
        schemaService.extractValue(data, "data.results.performance.loadTime")
      ).toBe(1250);
    });
    ...
    it("should handle multiple fallback paths", () => {
      ...
      const result = schemaService.extractValue(
        data,
        "data.results.performance.loadTime,data.results.loadTime"
      );
    ...
    it("should return null for non-existent paths", () => {
      ...
      expect(
        schemaService.extractValue(data, "data.nonexistent.path")
      ).toBeNull();
      expect(
        schemaService.extractValue(data, "completely.wrong.path")
      ).toBeNull();
    });
    ...
    it("should handle null and undefined values", () => {
      ...
      expect(
        schemaService.extractValue(data, "data.undefinedValue")
      ).toBeNull();
  ...
  describe("Data Formatting", () => {
    ...
    it("should use custom format templates", () => {
      const result = schemaService.formatValue(
        524288,
        "bytes",
        "{value} KB ({valueMB} MB)"
      );
