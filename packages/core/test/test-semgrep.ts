...
async function testSemgrepService() {
  ...
  const exampleFile = path.resolve(__dirname, '..', 'semgrep-rules', 'example-code.js');
  ...
  if (dirResult.success) {
    ...
    console.log(`   Found ${dirResult.stats.total_matches} issue(s) in ${dirResult.stats.files_scanned} file(s)`);
    
    // Show first 3 findings if any
    if (dirResult.matches.length > 0) {
      console.log('\n   First few findings:');
      dirResult.matches.slice(0, 3).forEach(match => {
        console.log(`   - [${match.severity}] ${match.rule_id} in ${match.path}:${match.start_line}`);
      });
    }
  } else {
