...
${dataTexts.map((text, i) => `  [${i}] "${text}"`).join('\n')}`;
  ...
  test.describe('Project Detection System', () => {
    test('should detect project languages and filter tools', async () => {
      console.log('🔍 Testing project detection and tool filtering...');
      ...
      console.log('🎯 Testing project detection command...');
      ...
      await vscode.window.keyboard.press('Enter');
      console.log('✅ Triggered project detection command');
      ...
      console.log(`📊 Found ${toolCount} tools in the tree`);
      
      if (toolCount > 0) {
        const toolTexts = await toolRows.allTextContents();
        console.log('📋 Available tools:', toolTexts.slice(0, 5));
        
        // Look for compatibility indicators
        const compatibleTools = toolTexts.filter(text => text.includes('✓ Compatible'));
        const incompatibleTools = toolTexts.filter(text => text.includes('⚠ Not compatible'));
        
        console.log(`✅ Found ${compatibleTools.length} compatible tools`);
        console.log(`⚠️ Found ${incompatibleTools.length} incompatible tools`);
      ...
      console.log('✅ Project detection and tool filtering working!');
    });

    test('should export project info for CLI consumption', async () => {
      console.log('🔍 Testing project info export for CLI...');
      
      // Workspace is already opened in beforeEach with 'test-workspace'
      
      // Test export project info command
      console.log('🎯 Testing export project info command...');
      await vscode.window.keyboard.press('Control+Shift+P');
      await vscode.window.waitForTimeout(1000);
      ...
      // Press Enter to execute
      await vscode.window.keyboard.press('Enter');
      console.log('✅ Triggered export project info command');
      ...
      console.log('✅ Project info export command executed successfully!');
    });
  });
}); 
