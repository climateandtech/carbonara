...
async function showCarbonaraMenu() {
  const items = [
    ...
    {
      label: UI_TEXT.MENU.ITEMS.REFRESH_HIGHLIGHTS.LABEL,
      description: UI_TEXT.MENU.ITEMS.REFRESH_HIGHLIGHTS.DESCRIPTION,
...
async function initProject() {
  ...
  if (cliAvailable) {
    ...
    promptInstallCli(
      "Project initialized locally. Install Carbonara CLI to unlock all features."
    );
  }
...
function promptInstallCli(message?: string) {
  const text =
    message ||
    "Carbonara CLI is not installed. Install it to enable advanced features.";
  vscode.window
    .showInformationMessage(text, "Install CLI", "Later")
    .then((selection) => {
    ...
    try {
      ...
      if (resolved.mode === "path") {
        ...
        if (resolved.path.endsWith(".js")) {
          child = spawn("node", [resolved.path, "--version"], {
            stdio: "ignore",
          });
        } else {
...
async function runCarbonaraCommand(
  args: string[],
  message: string
): Promise<void> {
  const resolved = resolveCarbonaraCli();
    ...
    vscode.window.withProgress(
      ...
      (progress, token) => {
        return new Promise<void>((progressResolve) => {
          ...
          if (resolved.mode === "path") {
            if (resolved.path.endsWith(".js")) {
              child = spawn("node", [resolved.path, ...args], {
                cwd,
                stdio: "pipe",
              });
            } else {
          ...
          const timeout = setTimeout(() => {
            if (finished) return;
            output.appendLine(
              "[Carbonara] Command timed out. Terminating process..."
            );
            try {
              child.kill();
            } catch {}
          ...
          token.onCancellationRequested(() => {
            output.appendLine(
              "[Carbonara] Operation cancelled by user. Killing process..."
            );
            try {
              child.kill();
            } catch {}
          ...
          child.on("close", (code) => {
            ...
            if (code === 0) {
              ...
              const errorMessage =
                errorBuf || `Command failed with exit code ${code}`;
          ...
          child.on("error", (err) => {
            ...
            clearTimeout(timeout);
            output.appendLine(
              `[Carbonara] Failed to start CLI: ${String(err)}`
            );
