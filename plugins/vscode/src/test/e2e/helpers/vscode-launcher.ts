...
import { UI_TEXT, SELECTORS } from "../../../constants/ui-text";
...
export class VSCodeLauncher {
   ...
   */
  private static detectVSCodePath(): {
  ...
  static async launch(
    workspaceFixture: WorkspaceFixture = "test-workspace"
  ...
  static async waitForExtension(window: Page, timeout = 20000): Promise<void> {
    ...
    try {
      ...
      await window.waitForSelector(
        'a[role="button"][aria-label="carbonara-statusbar"]',
        {
          state: "visible",
          timeout,
        }
      ...
      const alternatives = [
        () =>
          window.waitForSelector(
            '.statusbar-item[aria-label="carbonara-statusbar"]',
  ...
  static async selectFromCarbonaraMenu(
    window: Page,
    menuItemKey: keyof typeof UI_TEXT.MENU.ITEMS
  ): Promise<void> {
    // First open the menu
    ...
    const menuItem = window.locator(
      `${SELECTORS.QUICK_PICK.LIST_ROW}:has-text("${searchText}")`
    );
  ...
  static async clickStatusBarCarbonara(window: Page): Promise<void> {
    ...
    const tryClicks = [
      async () => {
        const el = await window.waitForSelector(
          '[aria-label="carbonara-statusbar"]',
          { timeout: 8000 }
        );
        await el?.click();
      },
      ...
      async () => {
        const el = await window.waitForSelector(
          'footer .statusbar-item-label[aria-label*="Carbonara"]',
          { timeout: 8000 }
        );
        await el?.click();
      },
  ...
  static async dismissDialogs(window: Page): Promise<void> {
    ...
    try {
      const reloadButton = window.locator(
