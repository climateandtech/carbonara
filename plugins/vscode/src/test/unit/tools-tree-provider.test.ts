  ...
  suite("Tool Detection Logic", () => {
    ...
    test("should detect external tool as installed when command succeeds", async () => {
      ...
      try {
        ...
        const isInstalled = await (provider as any).detectToolInstallation(
          mockTool
        );
        assert.strictEqual(
          isInstalled,
          true,
          "Should detect tool as installed when command succeeds"
        );
      } finally {
    ...
    test("should detect external tool as not installed when command fails", async () => {
      ...
      try {
        ...
        assert.strictEqual(
          isInstalled,
          false,
          "Should detect tool as not installed when command fails"
        );
      } finally {
        // Restore original method
        (provider as any).runCommand = originalRunCommand;
    ...
    test("should default to not installed for tools without command detection", async () => {
      ...
      const isInstalled1 = await (provider as any).detectToolInstallation(
        mockToolNoDetection
      );
      const isInstalled2 = await (provider as any).detectToolInstallation(
        mockToolWrongMethod
      );
      ...
      assert.strictEqual(
        isInstalled1,
        false,
        "Should default to not installed when no detection method"
      );
      assert.strictEqual(
        isInstalled2,
        false,
        'Should default to not installed when detection method is not "command"'
      );
    });
    ...
    test("should create correct tool items based on installation status", () => {
      ...
      const installedItem = new ToolItem(
        installedTool,
        vscode.TreeItemCollapsibleState.None
      );
      const uninstalledItem = new ToolItem(
        uninstalledTool,
        vscode.TreeItemCollapsibleState.None
      );
