...
let vscode;
test_1.test.describe("Semgrep Integration E2E Tests", () => {
    ...
    test_1.test.beforeEach(async () => {
        vscode = await vscode_launcher_1.VSCodeLauncher.launch("with-carbonara-project");
    ...
    (0, test_1.test)("should trigger Semgrep analysis and show results with diagnostics", async () => {
        // Open the test file
        ...
        await (0, test_1.expect)(editor).toBeVisible({ timeout: 5000 });
        ...
        const diagnosticMarkers = vscode.window.locator('.monaco-editor .squiggly-error, .monaco-editor .squiggly-warning, .monaco-editor .squiggly-info, .monaco-editor .cdr.squiggly-d-error, .monaco-editor .cdr.squiggly-d-warning');
        // Wait for diagnostics to appear
        await vscode.window.waitForTimeout(2000);
        // Check if any diagnostics are visible
        const diagnosticCount = await diagnosticMarkers.count();
        (0, test_1.expect)(diagnosticCount).toBeGreaterThan(0);
        ...
        if (diagnosticCount > 0) {
            ...
            const hoverPopup = vscode.window.locator('.monaco-hover:not(.hidden), .hover-contents').first();
            ...
            if (hoverVisible) {
                ...
                if (hoverContent) {
                    ...
                    const hasDiagnosticContent = hoverContent.toLowerCase().includes('semgrep') ||
                    ...
                    (0, test_1.expect)(hasDiagnosticContent).toBe(true);
                }
            }
            else {
