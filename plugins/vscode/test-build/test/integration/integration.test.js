...
suite("Carbonara Extension Integration Tests", () => {
    ...
    test("All required commands should be registered", async () => {
        ...
        const isVSCodeGeneratedCommand = (cmd) => {
            return treeViewPrefixes.some(prefix => vscodeGeneratedSuffixes.some(suffix => cmd === prefix + suffix));
        };
        const unexpectedCommands = carbonaraCommands.filter((cmd) => !expectedCommands.includes(cmd) && !isVSCodeGeneratedCommand(cmd));
        assert.strictEqual(unexpectedCommands.length, 0, `Unexpected commands registered: ${unexpectedCommands.join(", ")}. Please update the test if these are intentional.`);
    });
    test("Tree data providers should be registered", () => {
        ...
        assert.ok(views.find((v) => v.id === "carbonara.assessmentTree"), "Assessment tree should be defined");
        assert.ok(views.find((v) => v.id === "carbonara.dataTree"), "Data tree should be defined");
    });
    ...
    test("Activity bar contribution should be present", () => {
        ...
        const carbonaraContainer = viewsContainers.find((container) => container.id === "carbonara");
    ...
    test("Menu contributions should be present", () => {
        ...
        const assessmentMenus = menus["view/title"].filter((menu) => menu.when && menu.when.includes("carbonara.assessmentTree"));
        const dataMenus = menus["view/title"].filter((menu) => menu.when && menu.when.includes("carbonara.dataTree"));
    ...
    test("Extension should handle command execution gracefully", async function () {
        ...
        for (const command of safeCommands) {
            try {
                ...
                console.log(`Command ${command} failed gracefully:`, error.message);
